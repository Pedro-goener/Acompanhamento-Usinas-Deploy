...
df = load_and_prepare_data(db_config,'SELECT * FROM "performance_data_bet"')
df['Tempo'] = pd.to_datetime(df['Tempo'])

st.title('Dispersão Real X Esperado')
#Filtro de inversores
inversor = st.selectbox('Selecione o Inversor',df['Inversor'].unique())
df_filtrado_inv = df[df['Inversor'] == inversor]
#Filtro Temporal
